...
@endphp
@foreach($listaHorasXrut as $horasxRut)@endforeach <h1>{{$horasXrut->nombre}}</h1>
<table class="table">
    @foreach($listaHorasXrut->groupBy('fecha') as $fecha => $horasxRut)
        <tr>
            <td colspan="2"><h1>{{ $fecha }}</h1></td>
        ...
        @foreach($horasxRut as $hora)
            <tr>
                ...
                <td> @foreach($listaHorasXrut as $horasxRut)@endforeach<button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#exampleModal" onclick="obtenerValorSeleccionado({{$horasxRut->id_fecha}},'{{$horasxRut->fecha}}','{{$horasxRut->hora}}',{{$horasxRut->id_especialista}},'{{$horasxRut->nombre.' '.$horasxRut->apellido}}',{{$horasxRut->id_especialidad}} )">ad</button> </td>
