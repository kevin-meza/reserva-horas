    ...
    @foreach($listaHorasXrut->groupBy('fecha') as $fecha => $horasxRut)
        ...
        @foreach($horasxRut as $hora)
            <tr>
                ...
                <td> @foreach($listaHorasXrut as $horasxRut)@endforeach<button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#exampleModal" onclick="obtenerValorSeleccionado({{$horasxRut->id_fecha}},'{{$horasxRut->fecha}}','{{$horasxRut->hora}}',{{$horasxRut->id_especialista}},'{{$horasxRut->nombre.' '.$horasxRut->apellido}}',{{$horasxRut->id_especialidad}} )"></button> </td>
