...
<table class="table">
    @foreach($listaHorasXrut as $horasRut)
    @php
    $id_especialista=$horasRut->id_especialista;
    @endphp

            @if ($id_especialista != $prev_fecha)
            <!-- Imprimir el nombre del especialista -->
            <tr>
                <td><h1>{{$horasRut->nombre.' '.$horasRut->apellido}}</h1></td>


                {{-- <td>{{$horasxDia->apellido}}</td> --}}
                <td><h1>{{$horasRut->fecha}}</h1></td>
            </tr>
