...
class HorasDisponiblesController extends Controller
{
    ...
    public function store(Request $request)
    {
            ...
            if (isset($existencia)) {
                ...
                $datos=[
                 ...
                 reservas::insert($datos);
                    ...
                    return view('horas_disponibles/index',$listaEspecialistas,$listaEspecialidades)->with(["mensaje"=>"reserva realizada"]);
            }else{
        ...
        // return view('horas_disponibles/index',$listaEspecialistas,$listaEspecialidades)->with(["mensaje"=>"reserva realizada"]);
        return redirect()->route('horas_disponibles.index')->with(['mensaje' => 'Hora reservada',$listaEspecialistas,$listaEspecialidades]);
    }
    ...
    public function buscar(Request $request){
       ...
       //  $listaHorasXrut['listaHorasXrut'] = horas_disponibles::where('id_especialista','=',$id_especialista)->get();
        ...
        ->where('horas_disponibles.id_especialista','=',$id_especialista)
        ->where('horas_disponibles.id_estado','!=',2)
        ->whereDate('horas_disponibles.fecha', '>=', now()->format('Y-m-d'))
        ->get();
    ...
    public function buscarxDia(Request $request){
        ...
        $id_especialidad =  intval(request()->id_especialidad);//obtiene solo el numero
    ...
    ->select('horas_disponibles.*', 'especialistas.*')
    ->get();


    // print_r($listaHorasxDia);exit;
    return view('horas_disponibles/horas_dia', [
        'id_especialidad' => $id_especialidad,
         'listaHorasxDia' => $listaHorasxDia['listaHorasxDia']
    ]);
